...
if(menu == 'check'):
    try:
        if(os.path.exists('./out/9918.txt')):
            tvbox = LocalFile.read_LocalFile('./out/9918.txt').replace('\r','').replace('\n\n','\n')
        else:
            tvbox = LocalFile.read_LocalFile('./code/9918.json').replace('\r','').replace('\n\n','\n')
        ...
        LocalFile.write_LocalFile('./out/9918.txt', '"sites":[\r\n//Update:' + str(datetime.datetime.now()) + '\r\n' + addtv + '\r\n' + nsfw + '\r\n' + spare + '\r\n],')
    except Exception as ex:
